...
def main( argv ):
  ...
  # to check if there are 5 command line arguments
  # these arguments are year, name, number, gender, and rank
  if len(argv) < 5:
    print ( "Invalid Call -> ./FileReadPrograms/readAustralia.py -i <input file name> -f <output file name female> -m <output file name male>" )
    sys.exit(2)
  ...
  #checks if the command line arguments lead to a different file path
  #and displays an error message if it does
  ...
  with open(inputFilePath) as csvDataFile:
    ...
    # Original Format: Rank, Name, Number, Gender, Year
    
    #only read in years from 1990 - 2021 and male names
    for row in csvReader:
      if int(row[4]) >= 1990 and int(row[4]) <= 2021 and row[3] == "Male" :
        names.append(row[1].title())
        number.append(int(row[2]))
  ...
  with open(inputFilePath) as csvDataFile:
    ...
    #only read in years from 1990 - 2021 and female names
    for row in csvReader:
      if int(row[4]) >= 1990 and int(row[4]) <= 2021 and row[3] == "Female" :
        names.append(row[1].title())
        number.append(int(row[2]))
        year.append(int(row[4]))
        total = total + 1
